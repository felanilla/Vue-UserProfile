<template>
    <div class="user">
        <img class="user__share icon" src="../assets/share.svg" alt="Share">
        <div>
            <img class="user__avatar" src="https://api.adorable.io/avatars/70/harvey@adorable.png" alt="User">
            <p class="user__name">
                {{firstName}} {{lastName}}
                <img v-bind:src="imageSrc" v-on:click="addLike" :class="{liked: isLiked}" class="user__heart icon icon--hover" alt="Like">
            </p>
  ...
  methods: {
    addLike: function() {
      ...
      if (this.isLiked == true) {
        this.likes += 1
        this.imageSrc = './src/assets/heart-active.svg'
      } else {
        this.likes -= 1
